...
function pageInitScreen()
{
    ...
    TweenMax.to($("#button_prize"),0, { top: window.innerHeight / heightRatio - 176 - 544 });
...
function gameStart() {
    ...
    s_oRequestManager.sendRealRequest("theCarRaceInitExecute", g_sid, 'body={"date": "'+getTodayDate()+'"}', function (json) {
        ...
        if(json && json.open){
            ...
            _gMilesForShare = json.mileage;

            if ( json.open == 1 )
            {
                if (window.location.href.split('?')[0] != json.url && json.code != 3)
                    window.location.href = json.url;
            }

            if ( json.open == 2 )
            {
                $("mainmenu").css("background-image", 'url("sprites/newmain.jpg")');

                $("#button_yiche").css("display", "none");
                //$("#button_desc").css("display", "none");
                $("#button_play").css("display", "none");
                $("#button_prize").css("display", "none");
            }

            if ( json.open == 3 )
            {
                window.location.href = "http://sale.jd.com/m/act/PpcihWax6f3Z.html";
    ...
    $("#button_prize").off().click(function () {

        //mPing
        mPingFunction("M618FCHome_Slogan", location.href.split('?')[0]);
...
function showPrize(){
    ...
    TweenMax.set($("prize"), { scale: 0.5, opacity: 0, display: 'block' });
    TweenMax.to($("prize"), 0, { scale: 1, opacity: 1, ease: Quint.easeInOut });

    $("#okPrize").off().click(function () {

        mPingFunction("M618FCWarn_Close", location.href.split('?')[0]);

        TweenMax.to($("prize"), 0, { scale: 0.5, opacity: 0, ease: Quint.easeInOut, display: 'none' });
...
function showInsufficientFuel(callback) {
    ...
    this.show = function(totalMiles){

        var gapResult = getGap(totalMiles);
        var msg = gapResult && gapResult.award ? ("再跑<span class='tips_color'>" + (gapResult.miles - (totalMiles || 0)) + "</span>米，就有可能获得<br>" + gapResult.award + "元加油卡，再接再厉！") : "今日战绩很棒！后面还有很多对手，再接再厉";
        $("#awardTips").html(msg + "<br><span class='tips_font tips_no_oils'>(每日总成绩前3000名，获得加油卡,最高可获4666元加油卡)</span>");
...
function showRanking() {
    ...
    $("#prizelist").off().click(function () {
        location.href = "http://sale.jd.com/m/act/eNBsPvx4Gog.html";
        ...
        opacity: 0, onComplete: function () {
            ...
            s_oRequestManager.sendRealRequest("theCarRaceGetCurrentRanking", "", " ", function (json) {
                ...
                if (s_oMain.initGameByServerResult != undefined)
                {
                    $("#ranking_totalmiles").html(s_oMain.initGameByServerResult.mileage + "米");
                    if (s_oMain.initGameByServerResult.ranking >= 1)
                        $("#ranking_rank").html("第"+s_oMain.initGameByServerResult.ranking+"名");
                    else
                        $("#ranking_rank").html("前" + Math.floor(s_oMain.initGameByServerResult.ranking * 100) + "%");
                }
...
function showHistoryRanking(date, ret) {
    ...
    TweenMax.to($("#rankingtext"), 0, {
        opacity: 0, onComplete: function () {
            ...
            s_oRequestManager.sendRealRequest("theCarRaceGetHistoricalRanking", g_sid, 'body={"page":"' + rankingCurrentPage + '","date":"' + date + '"}', function (json) {
                TweenMax.to($("#rankingtext"), 0, { opacity: 1 });
                var html = "";

                $("#ranking_totalmiles").text((json.miles || "--")+"米");
                $("#ranking_rank").text("---");
...
//获取还差多少距离
function getGap(currentMiles){

    //var arr = [{miles:67510,award:100},{miles:87074,award:500},{miles:166522,award:3000},{miles:217035,award:4666},{miles:500457,award:null}];
    var arr = [{miles:81341,award:100},{miles:123874,award:200},{miles:277747,award:1200},{miles:494464,award:4666},{miles:500457,award:null}];
    if(!currentMiles)return arr[0];
    for(var i=0; i<arr.length;i++){
        if(arr[i].miles > currentMiles){
            return arr[i];
        }
    }
}
...
function showMilesData(json, miles){
    ...
    $("#endgame_tip").html("温馨提示：还有大量的用户向您的成绩呼啸而来，为了大奖继续加油吧！");
    if (json){
    ...
    if (isWeixinBrowser()) {
        ...
        if ( json != null && json.rpb) {
            ...
            if (json)
            {
                var gapResult = getGap(json &&json.mileage);
                var msg = gapResult && gapResult.award ? ("再跑<span class='tips_color'>"+(gapResult.miles - json.mileage)+"</span>米，就有可能获得"+gapResult.award+"元加油卡，再接再厉！") : "今日战绩很棒！后面还有很多对手，再接再厉";
                $("#endgame_message1").html(msg+"<br><span class='tips_font'>(每日总成绩前3000名，获得加油卡)</span>");
            }
        }
...
function showEndGame(json, miles) {
    ...
    $("#endGameGoIndex").off().click(function () {
        ...
        TweenMax.to($("#button_desc"), 0, { bottom: 1166, opacity: 1, ease: Quint.easeInOut });
        TweenMax.to($("#button_play"), 0, { bottom: 1236, opacity: 1, ease: Quint.easeInOut });
        TweenMax.to($("#button_prize"),0, { bottom: 1936, opacity:1, ease:Quint.easeInOut });
...
function shareFunction(){
    s_oRequestManager.sendRealRequest("theCarRaceWeiXinShareOil", g_sid, " ", function (json) {
        ...
        var content = "我在京东618飞车游戏中跑了"+_gMilesForShare+"米";
        var param = [encodeURIComponent("4666元加油卡就要到手，快来帮我加油！"),encodeURIComponent(content+"，众泰T600豪送的4666元加油卡就要到手!"),shareURL,"http://h5.m.jd.com/active/carrace/sprites/icon.jpg"];
